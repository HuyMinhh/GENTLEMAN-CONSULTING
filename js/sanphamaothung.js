...
// Hàm lọc sản phẩm thuộc loại "áo thun"
function filterProductsByCategory(products) {
  return products.filter(product => 
    product.category && (product.category.toLowerCase().includes("áo thun") || product.category.toLowerCase().includes("ao thun")) ||
    (product.name && (product.name.toLowerCase().includes("áo thun") || product.name.toLowerCase().includes("ao thun")))
  );
}
...
function displayProducts(sortOrder = "asc", startIndex = 0, limit = 6) {
  ...
  // Lọc sản phẩm thuộc loại "áo thun"
