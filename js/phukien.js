...
function filterProductsByCategory(products) {
  return products.filter(product => 
    (product.name && (
      product.name.toLowerCase().includes("Nón") || 
      product.name.toLowerCase().includes("Thắt lưng") || 
      product.name.toLowerCase().includes("Dày") ||
      product.name.toLowerCase().includes("Mắt kính") ||  
      product.name.toLowerCase().includes("Dép")
    )) ||
    (product.category && (
      product.category.toLowerCase().includes("Nón") || 
      product.category.toLowerCase().includes("Thắt lưng") || 
      product.category.toLowerCase().includes("Dày") ||
      product.category.toLowerCase().includes("Mắt kính") ||  
      product.category.toLowerCase().includes("Dép")
    ))
  );
}
...
function displayProducts(sortOrder = "asc", startIndex = 0, limit = 6) {
  ...
  if (filteredProducts.length === 0) {
    ...
    if (noMoreProductsMsg) noMoreProductsMsg.style.display = "none";
    return;
  }
