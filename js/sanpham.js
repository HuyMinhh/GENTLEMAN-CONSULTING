...
//       ...product,
//       quantity,
//       selectedSize: modal.selectedSize,
//       selectedColor: modal.selectedColor,
//     };
//     cart.push(cartItem);
...
//     if (totalDisplayed >= products.length || (totalDisplayed % 6 !== 0 && totalDisplayed > 0)) {
...
// // Hàm load thêm một hàng sản phẩm khi nhấn "Xem thêm"
...
//   displayProducts(sortOrder, startIndex, 6); // Load thêm 6 sản phẩm cho một hàng
// }
...
function updateCartCount() {
  const countCart = document.querySelector(".count-cart");
  if (!countCart) {
    console.error("Element with class 'count-cart' not found!");
    return;
  }
  const cart = JSON.parse(localStorage.getItem("cart")) || [];
  const total = cart.reduce((sum, item) => sum + (item.quantity || 0), 0);
  countCart.textContent = total > 0 ? total : "0";
  countCart.style.display = total > 0 ? "block" : "none"; // Ẩn khi tổng là 0
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Gọi hàm hiển thị sản phẩm ban đầu với 6 sản phẩm
  displayProducts();
  updateCartCount(); // Đảm bảo cập nhật số lượng giỏ hàng khi trang load
