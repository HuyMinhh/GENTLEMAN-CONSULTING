...
// Hàm lọc sản phẩm thuộc loại "áo sơ mi"
function filterProductsByCategory(products) {
  return products.filter(product => 
    product.category && (product.category.toLowerCase().includes("áo sơ mi") || product.category.toLowerCase().includes("ao so mi")) ||
    (product.name && (product.name.toLowerCase().includes("áo sơ mi") || product.name.toLowerCase().includes("ao so mi")))
  );
}
...
function displayProducts(sortOrder = "asc", startIndex = 0, limit = 6) {
  ...
  // Lọc sản phẩm thuộc loại "áo sơ mi"
